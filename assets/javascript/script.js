...
document.addEventListener('DOMContentLoaded', function() {
    // Initial setup or loading saved widgets
    loadSavedWidgets();
});
...
function createWidget(type) {
    ...
    switch (type) {
        case 'workout':
            widget.innerHTML = `
                <h3>Workout</h3>
                <p>Today's workout plan: 30 mins of HIIT</p>
                <button onclick="removeWidget(this)">Remove</button>
            `;
            break;
        case 'nutrition':
            widget.innerHTML = `
                <h3>Nutrition</h3>
                <p>Calories: 2200, Protein: 150g, Carbs: 200g, Fat: 70g</p>
                <button onclick="removeWidget(this)">Remove</button>
            `;
            break;
        case 'progress':
...
function saveWidgets() {
    const widgets = Array.from(document.querySelectorAll('.widget')).map(widget => widget.getAttribute('data-type'));
    localStorage.setItem('dashboardWidgets', JSON.stringify(widgets));
}

function loadSavedWidgets() {
    const savedWidgets = JSON.parse(localStorage.getItem('dashboardWidgets') || '[]');
    savedWidgets.forEach(type => {
        const widget = createWidget(type);
        document.getElementById('dashboard-widgets').appendChild(widget);
    });
}
